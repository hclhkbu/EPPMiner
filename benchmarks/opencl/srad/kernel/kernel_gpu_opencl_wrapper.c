...
//  sprintf(clOptions,"-I../../src");                                                    
  ...
  sprintf(clOptions + strlen(clOptions), " -DRD_WG_SIZE_0_0=%d", RD_WG_SIZE_0_0);
	...
	// execute main loop
	for (iter=-100; iter<niter; iter++){										// do for the number of iterations input parameter
		...
		if (iter == -1){
			if(timeRestrict){
				...
				printf("Estimate %lf s, Adjust ITER to be %d to meet time requirement %d secs.\n", tPerIter, niter, secs);
			}
			total_s = 0.0;
	...
	printf("\n");
        ...
        double averMsecs = total_s * 1000.0 / niter;
