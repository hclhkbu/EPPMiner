...
int main(int argc, char *argv[]) {
   ...
   */
  if (gpu_compute_cutoff_potential_lattice6overlap(gpu_lattice, cutoff, atom, 0,num_iter_control)) {
  ...
  /* Print output */
  if (argv[2]) {
    write_lattice_summary(argv[2], gpu_lattice);
  }
