...
int main(int argc, char* argv[]) {
	...
	printf("Base implementation of histogramming on mic.\n");
	...
	//printf("[DEBUG] numIterations=%d, image width=%d, image height=%d", numIterations, img_width, img_height);
	//fflush(0);
	const char* env_iter = getenv("ITER");
	int iteration = (env_iter != NULL) ? atoi(env_iter) : 1;
	printf("[ITERATION NUM]:%d\n", iteration);
	double time = omp_get_wtime();
	#pragma offload target(mic) in(iteration) in(img:length(img_width*img_height)) out(histo:length(histo_width*histo_height))
	{
		int iter;
