...
int main(int argc, char** argv) {
	...
	#pragma offload target(mic:0) in(dim, iteration) in(h_nzcnt:length(nzcnt_len)) in(h_ptr:length(depth)) \
	in(h_indices:length(len)) in(h_perm:length(dim)) in(h_data:length(len)) in(h_x_vector:length(dim))\
	out(h_Ax_vector:length(dim))
	{
		for(p=0;p<5000*iteration;p++)
		{
